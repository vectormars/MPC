...
%%%%%%%%%%%%%%%% without constraint %%%%%%%%%%%%%%%%
Delta_u2=zeros(1,N_sim);
x2=zeros(Nc,N_sim);
y2=zeros(1,N_sim);
u2=zeros(1,N_sim);
for kk=1:N_sim-1
    Delta_U2=inv(P_P+0.01*eye(Nc))*(P_Rs-P_F*x2(:,kk));
    Delta_u2(:,kk)=Delta_U2(1,1);
    x2(:,kk+1)=A*x2(:,kk)+B*Delta_u2(:,kk)+K_ob*(y2(kk)-C*x2(:,kk));
    y2(kk+1)=C*x2(:,kk+1);
    u2(kk+1)=u2(kk)+Delta_u2(kk);
end
